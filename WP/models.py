...
class User(AbstractUser):
    mob_no_regex = RegexValidator(regex='^\([0-9]{2}\)[1-9][0-9]{9}$|^[1-9][0-9]{9}$', message="Should be of 10 digits like (91)9999999999 or 9999999999")
...
class WaterPlant(models.Model):
    loc=models.ForeignKey(WaterPlantLoc,on_delete=models.CASCADE,default='0')
    ...
    operator_phone_number=models.CharField(validators=[phone_regex],max_length=10)
    incharge=models.ForeignKey(User,null=True,blank=True,on_delete=models.DO_NOTHING)
    plant_type=models.CharField(choices=TYPE_CHOICES,max_length=15,null=True,blank=True)
